...
#include "YASarch.h"
#include "YASarchMachineFunctionInfo.h"
...
static const MCPhysReg ArgGPRs[] = {YASarch::R9, YASarch::R10, YASarch::R11, YASarch::R12};

void YASarchTargetLowering::ReplaceNodeResults(SDNode *N,
...
YASarchTargetLowering::YASarchTargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(YASarch::R1);
...
#include "YASarchGenCallingConv.inc"
...
SDValue YASarchTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_YASarch);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, YASarch::R1, PtrVT);
      SDValue Address =
  ...
  Chain = DAG.getNode(YASarchISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_YASarch);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
                                   ...
                                   const YASarchSubtarget &Subtarget) {
...
static SDValue unpackFromRegLoc(SelectionDAG &DAG, SDValue Chain,
                                const CCValAssign &VA, const SDLoc &DL,
                                const YASarchTargetLowering &TLI) {
  MachineFunction &MF = DAG.getMachineFunction();
...
/// YASarch formal arguments implementation
SDValue YASarchTargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_YASarch);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &YASarch::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    YASarchFunctionInfo *UFI = MF.getInfo<YASarchFunctionInfo>();
...
bool YASarchTargetLowering::CanLowerReturn(
  ...
  if (!CCInfo.CheckReturn(Outs, RetCC_YASarch))
    return false;
...
SDValue
YASarchTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
  ...
  const YASarchSubtarget &STI = MF.getSubtarget<YASarchSubtarget>();
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_YASarch);
  ...
  return DAG.getNode(YASarchISD::RET, DL, MVT::Other, RetOps);
}
...
SDValue YASarchTargetLowering::PerformDAGCombine(SDNode *N,
...
bool YASarchTargetLowering::isLegalAddressingMode(const DataLayout &DL,
...
unsigned YASarchTargetLowering::getIsdOpIncCmp(ISD::CondCode CCVal) const {
  switch (CCVal) {
  default:
